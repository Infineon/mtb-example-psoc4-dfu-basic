...
#define DFU_SESSION_TIMEOUT_MS                     (10u)
...
int main(void)
{
    ...
    */
    ...
    __enable_irq();
        ...
        #if (TRANSPORT_I2C)      /*Depending on the transport defined in makefile, Initialize the DFU communication;*/
            Cy_DFU_TransportStart(CY_DFU_I2C);
        #elif (TRANSPORT_UART)
            Cy_DFU_TransportStart(CY_DFU_UART);
        #else
            for (;;);       /*Wrong Transport selected in the Makefile; Do not proceed further*/
        #endif

    #else
        Cy_DFU_TransportStart(CY_DFU_I2C);  /*PSoC 4000T supports only I2C as the transport for DFU*/
    #endif
    ...
    for (;;)
    {
        ...
        else if (state == CY_DFU_STATE_UPDATING)
        {
           ...
           /* if no command has been received during 5 seconds 
            * defined in the macro DFU_COMMAND_TIMEOUT_MS when the loading
...
static cy_en_dfu_status_t handle_metadata(cy_stc_dfu_params_t *params)
{
    ...
    else
    {
        ...
        if (status != CY_DFU_SUCCESS)
        {
            const uint32_t elfStartAddress = CY_DFU_APP0_VERIFY_START;
            const uint32_t elfAppSize      = CY_DFU_APP0_VERIFY_LENGTH;
